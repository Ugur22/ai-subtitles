...
@app.delete("/api/speaker/{speaker_name}")
async def delete_speaker(speaker_name: str):
    ...
    try:
        ...
        if success:
            return {"success": True, "message": f"Removed speaker: {speaker_name}"}
        else:
...
@app.post("/api/transcription/{video_hash}/auto_identify_speakers")
async def auto_identify_speakers(video_hash: str, threshold: float = 0.7):
    """
    ...
    try:
        ...
        print(f"Error in auto speaker identification: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
