...
export const TranscriptionUpload: React.FC<TranscriptionUploadProps> = ({
  ...
  const [chatInitialInput, setChatInitialInput] = useState<string | null>(null);
  ...
  const handleAskAboutMoment = () => {
    const timestamp = secondsToTimeString(currentTime);
    const activeSegment = displayedSegments.find((seg) => {
      const s = convertTimeToSeconds(seg.start_time);
      const e = convertTimeToSeconds(seg.end_time);
      return currentTime >= s && currentTime <= e;
    });

    let question: string;
    if (activeSegment?.speaker && activeSegment?.text) {
      const speaker = formatSpeakerLabel(activeSegment.speaker);
      const snippet =
        activeSegment.text.length > 80
          ? activeSegment.text.substring(0, 80) + "..."
          : activeSegment.text;
      question = `What's happening at [${timestamp}]? ${speaker} says: "${snippet}"`;
    } else {
      question = `What's happening at [${timestamp}]?`;
    }

    setShowChat(true);
    setShowSummary(false);
    setChatInitialInput(question);
  };
                      ...
                      <>
                        <div className="flex items-center px-4 py-2 gap-4">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-200 hover:text-white bg-white/10 hover:bg-white/20 rounded-lg transition-all"
                            title="Ask about this moment in chat"
                          >
                    ...
                    {showChat && (
                      <div className="h-full">
                        <ChatPanel
                          ...
                          initialInput={chatInitialInput ?? undefined}
