...
export const TranscriptDisplay = ({ transcription }: TranscriptDisplayProps) => {
  const [displayMode, setDisplayMode] = useState<'segments' | 'words'>('segments');
    ...
    <div className="p-4">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`px-2 py-1 text-xs font-medium rounded-full border ${speakerColorClasses}`}>
                  {segment.speaker}
                </span>
              </div>
              ...
              {displayMode === 'segments' ? (
                ...
                <div className="flex flex-wrap gap-1">
                  {segment.words?.map((word, wordIndex) => (
                    ...
                    >
                      <span className={`text-base hover:bg-gray-100 rounded px-1 py-0.5 cursor-pointer ${getSpeakerColor(word.speaker).split(' ')[0]}`}>
                        {word.word}
                      </span>
      ...
      <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="text-xl font-medium">{transcription.segments.length}</div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="text-xl font-medium capitalize">{transcription.language}</div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
