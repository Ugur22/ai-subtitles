...
|-----------|----------|-----|
| Backend API | Cloud Run | `https://ai-subs-backend-1052285886390.us-central1.run.app` |
| Frontend | Netlify | `https://ai-subs.netlify.app` |
...
gcloud run services update ai-subs-backend \
  --region=us-central1 \
  --update-env-vars='CORS_ORIGINS=["https://ai-subs.netlify.app","http://localhost:5173"]'
```
...
| Key | Value |
|-----|-------|
| `VITE_API_URL` | `https://ai-subs-backend-1052285886390.us-central1.run.app` |
...
netlify env:set VITE_API_URL "https://ai-subs-backend-1052285886390.us-central1.run.app"
```
...
gcloud run deploy ai-subs-backend \
  ...
  --cpu=2 \
  --timeout=3600 \
  --set-env-vars='CORS_ORIGINS=["https://ai-subs.netlify.app","http://localhost:5173"]' \
...
curl -s https://ai-subs-backend-1052285886390.us-central1.run.app/health | jq

# Test CORS headers
curl -s -I -X OPTIONS \
  -H "Origin: https://ai-subs.netlify.app" \
  -H "Access-Control-Request-Method: POST" \
  https://ai-subs-backend-1052285886390.us-central1.run.app/api/transcribe
```
...
|----------|-------|-------------|
| `CORS_ORIGINS` | `["https://ai-subs.netlify.app","http://localhost:5173"]` | Allowed origins for CORS |
...
gcloud run services update ai-subs-backend \
  --region=us-central1 \
  --set-env-vars='
CORS_ORIGINS=["https://ai-subs.netlify.app","http://localhost:5173"],
